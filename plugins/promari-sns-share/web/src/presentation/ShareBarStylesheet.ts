 ...
 */
import type { SecondaryStyle, Shape, ShareSettings, Size } from '../application/ShareSettings.ts';
...
const radius = (shape: Shape, destination: string): string =>
  ({ pill: '9999px', rounded: '6px', square: '0', official: destination === 'x' ? '9999px' : '3px' })[shape];
...
const build = ({ appearance, style, floating }: ShareSettings): string => {
...
export class ShareBarStylesheet {
  static build(config: ShareSettings): string {
    return build(config);
  }
}
