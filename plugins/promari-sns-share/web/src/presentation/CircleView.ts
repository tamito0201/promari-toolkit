...
import type { ShareBarViewModel, ButtonViewModel } from '../application/BuildShareBarUseCase.ts';
...
const button = (b: ButtonViewModel, attribute: string): string => {
  const name = Html.escape(labels[b.key] ?? b.label);
  const content = `${icon(b.key, b.icon)}<span class="caption">${name}</span>`;
  const attrs = `class="social" data-key="${Html.escape(b.key)}" ${attribute}="${Html.escape(b.key)}" aria-label="${Html.escape(b.tooltip)}"`;
  return b.action === 'copy' || b.action === 'native'
    ? `<button type="button" ${attrs}>${content}</button>`
    : `<a ${attrs} href="${Html.escape(b.href)}"${b.newTab ? ' target="_blank"' : ''} rel="noopener noreferrer${b.nofollow ? ' nofollow' : ''}">${content}</a>`;
};
const render = (vm: ShareBarViewModel, attribute: string, caption: string, like: boolean): string => {
  ...
  return `<style>${circleCss}</style><div class="circle ${like ? 'with-like' : ''}" role="group" aria-label="${Html.escape(vm.groupLabel)}">${caption ? `<div class="intro"><small>PASS IT ON</small><span>${Html.escape(caption)}</span></div>` : ''}<div class="row">${reaction}<div class="socials">${vm.primary.map(b => button(b, attribute)).join('')}${more}</div></div><p class="status" role="status" aria-live="polite"></p></div>`;
};
const circleCss = `
